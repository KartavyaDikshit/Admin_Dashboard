...
function ApiUsageContent() {
    ...
    useEffect(() => {
        ...
        const fetchLogs = async () => {
            setLoading(true);
            try {
                const response = await fetch(`/api/analytics/api-usage?page=${page}`);
                ...
                if (response.ok) {
                    ...
                    toast.error(data.error || 'Failed to load API usage logs');
                }
            } catch {
    ...
    return (
        <div className="space-y-8">
            <h1 className="text-2xl font-semibold">API & AI Usage Dashboard</h1>

            {summary && (
                <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
                    <StatCard title="Total API Calls" value={summary.totalLogs.toLocaleString()} />
                    <StatCard title="Total Cost" value={`$${Number(summary.totalCost).toFixed(4)}`} />
                    <StatCard title="Total Tokens" value={summary.totalTokens.toLocaleString()} />
